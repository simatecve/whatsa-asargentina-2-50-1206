...
export const BotToggleButton = ({
  ...
  const refreshStatus = async () => {
    try {
      ...
      console.log('Bot status refreshed from DB:', {
        data,
        isActive
      });
    } catch (error) {
  ...
  return <div className="flex flex-col items-center space-y-1 mx-[15px]">
      <span className="text-xs text-muted-foreground text-center">
        {isDisabled ? 'Cambiando...' : `Bot ${botActive ? 'ON' : 'OFF'}`}
      </span>
      <div className="flex items-center space-x-1">
        {botActive ? <Bot className="h-3 w-3 text-green-600" /> : <BotOff className="h-3 w-3 text-gray-400" />}
        <Switch checked={botActive} onCheckedChange={handleToggle} disabled={isDisabled} className="h-4 w-7" />
      </div>
    </div>;
};